// ========== CONFIG ==========
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxJJky5D931kXjFk_4zedpcSWmUWGiAftUklNBUJl9dNG8fdqqkrEb64zk1bZy8sj0t/exec';
...
function extractMainText(html) {
    ...
    const selectors = ['article', 'main', '[role="main"]', '.mw-parser-output', '#content', '#main-content', '.post-content', '.entry-content'];
    let target = null;
    for (let sel of selectors) { target = doc.querySelector(sel); if (target) break; }
    if (target) { let text = target.innerText; text = text.replace(/\s+/g, ' ').trim(); return text.slice(0, 5700); }
    let text = doc.body.innerText;
    text = text.replace(/\s+/g, ' ').trim();
    return text.slice(0, 5700);
}
...
async function handleInput(value) {
    ...
    if (urlPattern.test(trimmed)) {
        addressBar.textContent = trimmed.replace(/^https?:\/\//, '').split('/')[0];
        showLoading();
        try {
            const fetched = await fetchArticleText(trimmed);
            mainInput.value = fetched;
            await summarizeText(fetched);
        } catch(err) {
            displaySummary(null, err.message);
        }
    } else {
...
mainInput.addEventListener('input', () => {
    // Auto-detect and summarize when user stops typing (debounced)
